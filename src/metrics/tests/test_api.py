...
@pytest.mark.django_db(databases=['default', 'clickhouse'], transaction=True)
class TestMetricValueListView:
    ...
    def test_retrieve_metric_value_list(self, client, metrics, metric_values):
        ...
        """
        metric_to_test = metrics[0].id
        METRIC_VALUES_URL = get_metric_values_url(metric_to_test)
        res = client.get(METRIC_VALUES_URL)

        metric_values_from_db = models.MetricValue.objects.filter(metric_id=metric_to_test)
    ...
    def test_retrieve_metric_value_list_filter_by_h3(self, metric_values, client):
        ...
        metric_values_from_db = models.MetricValue.objects.filter(metric_id=metric_to_test, h3_index=h3_index)
        serialized = serializers.MetricValueSerializer(metric_values_from_db, many=True)
        assert res.status_code == status.HTTP_200_OK
        assert res.data['count'] == 2
        for res_i in res.data['results']:
    ...
    def test_retrieve_metric_value_list_filter_by_time_range(self, metric_values, client):
        ...
        metric_values_from_db = models.MetricValue.objects.filter(metric_id=metric_to_test, time__range=(time, time))
        serialized = serializers.MetricValueSerializer(metric_values_from_db, many=True)
        assert res.status_code == status.HTTP_200_OK
        assert res.data['count'] == 2
        for res_i in res.data['results']:
    ...
    def test_retrieve_metric_value_list_filter_by_lat_long(self, metric_values, client):
        ...
        metric_values_from_db = models.MetricValue.objects.filter(metric_id=metric_to_test, h3_index=utils.h3_index1)
        serialized = serializers.MetricValueSerializer(metric_values_from_db, many=True)
        assert res.status_code == status.HTTP_200_OK
        assert res.data['count'] == 2
        for res_i in res.data['results']:
    ...
    def test_retrieve_metric_value_list_number_of_queries(self, metric_values, client, connections):
        """
        ...
        assert len(connections['clickhouse'].queries + connections['default'].queries) == 3
